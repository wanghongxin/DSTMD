...
% 函数说明
% 该函数用于实现对于 ESTMD 输出结果的聚类
% 首先沿 Z 轴进行 Max 操作，寻求运动方向
% 然后对 Max 操作后的结果进行聚类


% 导入变量 Max_Operation_DS_STMD_Outputs
if ~exist('Max_Operation_ESTMD_Outputs','var')
...
% 生成存储聚类结果的 cell
NumFrame_Clustering = length(Max_Operation_ESTMD_Outputs);
...
for j = 1:NumFrame_Clustering
    ...
    ModelOutputs = Max_Operation_ESTMD_Outputs{j};
    Clustering_Points_Matrix = [];
    %Clustering_Points_Num_Matrix_Each_Direction = zeros(1,8);
    Index_Clustering = 0;
    
    for k = 1:H_Clustering
        
        % 寻找大于阈值的点
        [IndX,IndY] = find(ModelOutputs(:,:,k)>ESTMD_Detection_Threshold);
        % 去掉一些边界点
...
save(file,'Clustering_Results','NumFrame_Clustering','ESTMD_Detection_Threshold','-v7.3')
