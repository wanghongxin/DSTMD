...
% 函数说明
% 该函数用于实现对于 TQD ON 输出结果的聚类
% 首先沿 Z 轴进行 Max 操作，寻求运动方向
% 然后对 Max 操作后的结果进行聚类


% 导入变量 Max_Operation_DS_STMD_Outputs
if ~exist('Max_Operation_TQD_OFF_Outputs','var')
...
% 生成存储聚类结果的 cell
NumFrame_Clustering = length(Max_Operation_TQD_OFF_Outputs);
...
for j = 1:NumFrame_Clustering
    ...
    ModelOutputs = Max_Operation_TQD_OFF_Outputs{j};
    Clustering_Points_Matrix = zeros(M_Clustering*N_Clustering*H_Clustering,3);
    ...
    for k = 1:H_Clustering
        ...
        % 寻找大于阈值的点
        [IndX,IndY] = find(ModelOutputs(:,:,k)>TQD_OFF_Detection_Threshold);
        % 去掉一些边界点
        ...
        if Cluster_Num>0
            ...
            % 存储类中心
            Clustering_Points_Matrix((Index_Clustering+1):(Index_Clustering+Cluster_Num),:) = [Cluster_Center,k*ones(Cluster_Num,1)];
...
file = [Parameter_File.folder_Global,'/',strcat('Clustering_Results-TQD-OFF-Detection-Threshold-',num2str(TQD_OFF_Detection_Threshold),'.mat')];
save(file,'Clustering_Results','NumFrame_Clustering','TQD_OFF_Detection_Threshold','-v7.3')
