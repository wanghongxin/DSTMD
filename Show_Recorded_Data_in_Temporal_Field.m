...
PlotEndFrame_TemporalField = Parameter_File.EndFrame - Parameter_File.StartRecordFrame+1;
%PlotEndFrame_TemporalField = Parameter_File.EndFrame;
...
All_ON_Channel_LI = zeros(M,N,Num);
All_OFF_Channel_LI = zeros(M,N,Num);

All_ESTMD_Delayed_OFF_Channel = zeros(M,N,Num); 

All_TQD_Delayed_ON_Channel = zeros(M,N,Num);
All_TQD_Delayed_OFF_Channel = zeros(M,N,Num);

All_TQD_Delayed_ON_Channel_LI = zeros(M,N,Num);
All_TQD_Delayed_OFF_Channel_LI = zeros(M,N,Num);
...
%        All_DS_STMD_CorrelationOutput(:,:,k) = StageValue(:,:,1);
     ...
     All_ON_Channel_LI(:,:,k) = RecordedData{k}.ON_Channel_LI;
     All_OFF_Channel_LI(:,:,k) = RecordedData{k}.OFF_Channel_LI;
     
     
     All_ESTMD_Delayed_OFF_Channel(:,:,k) = RecordedData{k}.ESTMD_Delayed_OFF_Channel;
     ...
     All_TQD_Delayed_ON_Channel_LI(:,:,k) = RecordedData{k}.TQD_Delayed_ON_Channel_LI;
     All_TQD_Delayed_OFF_Channel_LI(:,:,k) = RecordedData{k}.TQD_Delayed_OFF_Channel_LI;
...
Y_0 = 500-round((Parameter_File.TargetVelocity/Parameter_File.VideoSamplingFrequency)*(100+Parameter_File.StartRecordFrame));                
%Y_0 = 352;
% 将数据转化为向量
...
% ESTMD
% ON and OFF
SinglePixel_ON_Channel_LI = All_ON_Channel_LI(X_0,Y_0,:);
SinglePixel_ON_Channel_LI = reshape(SinglePixel_ON_Channel_LI,Num,1);

SinglePixel_All_ESTMD_Delayed_OFF_Channel = All_ESTMD_Delayed_OFF_Channel(X_0,Y_0,:);
SinglePixel_All_ESTMD_Delayed_OFF_Channel = reshape(SinglePixel_All_ESTMD_Delayed_OFF_Channel,Num,1);


figure
plot(PlotStratFrame_TemporalField:PlotEndFrame_TemporalField,SinglePixel_ON_Channel_LI,'color','r','DisPlayName','ON Channel')
hold on 
plot(PlotStratFrame_TemporalField:PlotEndFrame_TemporalField,SinglePixel_All_ESTMD_Delayed_OFF_Channel,'color','b','DisPlayName','ESTMD Delayed Channel OFF')
grid on 
legend('show')
title('ON and OFF Channels (ESTMD)')
...
if strcmp(Parameter_File.MotionMode,'BackgroundStationary')
    TQD_Pixel_Dist = Parameter_Fun.TQD_Dist;
elseif strcmp(Parameter_File.MotionMode,'OppositeDirection')
    TQD_Pixel_Dist = -Parameter_Fun.TQD_Dist;
end
% 此处默认白背景下，物体向左运动
% 混杂背景下，背景向右运动
...
SinglePixel_OFF_Channel_LI = All_OFF_Channel_LI(X_0,Y_0,:);
SinglePixel_OFF_Channel_LI = reshape(SinglePixel_OFF_Channel_LI,Num,1);

% ON
% 这里的 +TQD_Dist 或 -TQD_Dist 应当根据背景的运动方向来选择，在我们的数据集中，对于 Cluttered Backgroud
% 选择 -TQD_Dist， 对于 White Background 选择 +TQD_Dist
SinglePixel_All_TQD_Delayed_ON_Channel_LI = All_TQD_Delayed_ON_Channel_LI(X_0,Y_0+TQD_Pixel_Dist,:);
SinglePixel_All_TQD_Delayed_ON_Channel_LI = reshape(SinglePixel_All_TQD_Delayed_ON_Channel_LI,Num,1);
% OFF
SinglePixel_All_TQD_Delayed_OFF_Channel_LI = All_TQD_Delayed_OFF_Channel_LI(X_0,Y_0+TQD_Pixel_Dist,:);
SinglePixel_All_TQD_Delayed_OFF_Channel_LI = reshape(SinglePixel_All_TQD_Delayed_OFF_Channel_LI,Num,1);


MarkerEdgeColors = jet(4);
figure
plot(PlotStratFrame_TemporalField:PlotEndFrame_TemporalField,SinglePixel_ON_Channel_LI,'color',MarkerEdgeColors(1,:),'DisPlayName','ON Channel (LI)')
hold on 
plot(PlotStratFrame_TemporalField:PlotEndFrame_TemporalField,SinglePixel_OFF_Channel_LI,'color',MarkerEdgeColors(2,:),'DisPlayName','OFF Channel (LI)')
hold on
plot(PlotStratFrame_TemporalField:PlotEndFrame_TemporalField,SinglePixel_All_TQD_Delayed_ON_Channel_LI,'color',MarkerEdgeColors(3,:),'DisPlayName','TQD Delayed Channel ON (LI)')
hold on 
plot(PlotStratFrame_TemporalField:PlotEndFrame_TemporalField,SinglePixel_All_TQD_Delayed_OFF_Channel_LI,'color',MarkerEdgeColors(4,:),'DisPlayName','TQD Delayed Channel OFF (LI)')
grid on 
legend('show')
title('ON and OFF Channels (TQD LI)')
...
% 这里默认小物体始终向左运动
SinglePixel_All_DS_STMD_Delayed_ON_Channel = All_DS_STMD_Delayed_ON_Channel(X_0,Y_0+Parameter_Fun.DS_STMD_Dist,:);
...
SinglePixel_DS_STMD_Delayed_OFF_Channel_2 = All_DS_STMD_Delayed_OFF_Channel_2(X_0,Y_0+Parameter_Fun.DS_STMD_Dist,:);
