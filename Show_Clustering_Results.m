...
if ~exist('Clustering_Results','var')
    ...
    File_Max = strcat(Parameter_File.folder_Global,'\','Clustering_Results-Detection-Threshold-',num2str(DS_STMD_Detection_Threshold),'.mat');
    load(File_Max)
...
for j = Show_Start_Frame:Show_End_Frame
    ...
    %imwrite(frame2im(getframe(gcf)),strcat(Parameter_File.folder_Global,'\','Tracking-Result-Background-Noise-Trace-Thershold-',num2str(DS_STMD_Detection_Threshold),'-',num2str(j),'.jpg'))
...
%存储图像
imwrite(frame2im(getframe(gcf)),strcat(Parameter_File.folder_Global,'\','Tracking-Result-Thershold-',num2str(DS_STMD_Detection_Threshold),'-',num2str(j),'.fig'))
