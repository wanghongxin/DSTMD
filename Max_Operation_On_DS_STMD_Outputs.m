...
if ~exist(file,'file')
    if ~exist('DS_STMD_Outputs_After_Inhibition_Along_Theta_Axis_All','var')
        ...
        if ~exist(strcat(Parameter_File.folder_Global,'\','DS_STMD_Outputs_After_Inhibition_Along_Theta_Axis_All.mat'),'file')
            ...
            if ~exist('RecordedData_DS_STMD','var')
                File_Max = [Parameter_File.folder_Global,'\','Recorded-Data-DS-STMD.mat'];
                load(File_Max)
            end
            
            NumFrame_Clustering = length(RecordedData_DS_STMD);
            DS_STMD_Outputs_After_Inhibition_Along_Theta_Axis_All = cell(1,NumFrame_Clustering);
            
            for j = 1:NumFrame_Clustering
                
                DS_STMD_Outputs_After_Inhibition_Along_Theta_Axis_All{j} = RecordedData_DS_STMD{j}.DS_STMD_Outputs_After_Inhibition_Along_Theta_Axis;
