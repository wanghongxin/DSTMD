...
timedLog('Start Calcualte TQD OFF -- Detection Rate and False Alarm Rate...')
%% Main Function
...
if ~exist('Clustering_Results','var')
    % 导入聚类
    File = [Parameter_File.folder_Global,'/',strcat('Clustering_Results-TQD-OFF-Detection-Threshold-',num2str(TQD_Detection_Threshold),'.mat')];
    load(File)
end
...
% 
Detected_Target_Num_TQD_OFF = 0;
False_Positive_Num_TQD_OFF= 0;


for kk = 1:NumFrame_Clustering
    
    Detected_Target_Num_TQD_OFF_Each_Frame = 0;
    ...
    for jj = 1:Target_Num
        ...
        if Smallest_Dist<Distance_Threshold
            ...
            Detected_Target_Num_TQD_OFF = Detected_Target_Num_TQD_OFF +1;
            Detected_Target_Num_TQD_OFF_Each_Frame = Detected_Target_Num_TQD_OFF_Each_Frame + 1;
        end
    end
    
    False_Positive_Num_TQD_OFF = False_Positive_Num_TQD_OFF + (All_Detected_Object_Num-Detected_Target_Num_TQD_OFF_Each_Frame);
    
end


Detection_Rate_TQD_OFF =  Detected_Target_Num_TQD_OFF/(Target_Num*NumFrame_Clustering);
False_Alarm_Rate_TQD_OFF = False_Positive_Num_TQD_OFF/NumFrame_Clustering;
