...
if ~exist('Max_Operation_TQD_ON_Outputs','var')
...
% 生成存储聚类结果的 cell
NumFrame_Clustering = length(Max_Operation_TQD_ON_Outputs);
...
for j = 1:NumFrame_Clustering
    ...
    ModelOutputs = Max_Operation_TQD_ON_Outputs{j};
    ...
    for k = 1:H_Clustering
        ...
        % 寻找大于阈值的点
        [IndX,IndY] = find(ModelOutputs(:,:,k)>TQD_ON_Detection_Threshold);
        % 去掉一些边界点
...
file = [Parameter_File.folder_Global,'/',strcat('Clustering_Results-TQD-ON-Detection-Threshold-',num2str(TQD_ON_Detection_Threshold),'.mat')];
save(file,'Clustering_Results','NumFrame_Clustering','TQD_ON_Detection_Threshold','-v7.3')
