...
timedLog('Start Calcualte ESTMD Model -- Detection Rate and False Alarm Rate...')
%% Main Function
...
if ~exist('Clustering_Results','var')
    % 导入聚类
    File = [Parameter_File.folder_Global,'/',strcat('Clustering_Results-ESTMD-Detection-Threshold-',num2str(ESTMD_Detection_Threshold),'.mat')];
    load(File)
end
...
% 背景的运动方向
% 
Detected_Target_Num_ESTMD = 0;
False_Positive_Num_ESTMD= 0;


for kk = 1:NumFrame_Clustering
    
    Detected_Target_Num_ESTMD_Each_Frame = 0;
    ...
    for jj = 1:Target_Num
        ...
        if Smallest_Dist<Distance_Threshold
            Detected_Target_Num_ESTMD = Detected_Target_Num_ESTMD +1;
            Detected_Target_Num_ESTMD_Each_Frame = Detected_Target_Num_ESTMD_Each_Frame + 1;
        end
    end
    
    False_Positive_Num_ESTMD = False_Positive_Num_ESTMD + (All_Detected_Object_Num-Detected_Target_Num_ESTMD_Each_Frame);
    
end


Detection_Rate_ESTMD =  Detected_Target_Num_ESTMD/(Target_Num*NumFrame_Clustering);
False_Alarm_Rate_ESTMD = False_Positive_Num_ESTMD/NumFrame_Clustering;
